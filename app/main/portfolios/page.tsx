...
export default function PortfoliosPage() {
  ...
  return (
    <div>
      <h1 className="text-xl font-bold text-white mb-4">Portfolios</h1>
            ...
            <div key={entry.player.id} className={`rounded-xl border overflow-hidden ${isMe ? 'border-[#00c805]/30' : 'border-white/[0.08]'} bg-[#18110D]`}>
              ...
              <div className={`px-4 py-3 flex items-center justify-between ${isMe ? 'bg-[#00c805]/[0.06]' : 'bg-white/[0.03]'}`}>
                ...
                <div className="flex items-center gap-2 text-sm">
                  ...
                  <span className="font-bold text-white">{formatCoins(entry.current_value)}</span>
                </div>
              </div>
                  ...
                  <tbody>
                    {rows.map(row => (
                      <tr key={row.holding.id} className="border-b border-white/[0.04] last:border-0">
                        ...
                        <td className="text-right px-4 py-2.5 font-bold text-white">{formatCoins(row.total_value)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
